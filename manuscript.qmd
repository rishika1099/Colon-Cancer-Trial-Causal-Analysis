...
5-fluorouracil (Lev+5FU) as an effective post-operative chemotherapy for
resected stage B/C colon cancer. The trial reports a Cox hazard ratio of
approximately 0.67 for death in the Lev+5FU arm relative to observation.
That number is a marginal estimate; on its own, it does not describe the
absolute-scale benefit, heterogeneity across patients, the mechanism
through which treatment acts, or the populations to which the estimate
may extend.

This project re-analyzes the same dataset (n=929; available as
`survival::colon` or `ForCausality::Colon_df` in R) using five causal-inference
methods that are nested in the sense that each adds one additional
structural concern on top of the previous one:

| Q | Question | Cell |
|----|----------|------|
| Q1 | Randomized ATE | Causal, no selection |
| Q2 | Back-door identification (forget randomization) | Causal, no selection (obs.) |
| Q3 | Heterogeneous treatment effects (CATE) | Causal, no selection (conditional) |
| Q4 | Mediation through recurrence | Causal, no selection (mediator) |
| Q5 | Transportability to SEER 1990 | Causal, with selection |

The specification sheet — `00_estimands.qmd` in the replication package
— states each estimand in `do(·)` or potential-outcomes notation, lists
the graphical and non-graphical identification assumptions, names the
estimator, and notes the failure mode if the assumption is wrong. Every
notebook reads from this sheet.
...
The five-question structure brings out three things that the published
hazard ratio alone does not. Moertel's 0.67 is reproduced (we obtain 0.69).
The estimand is written down before the estimator, the identifying graph
is drawn before the regression is fit, the bad-control comparison sits
next to the correct analysis, and every causal claim carries a matched
sensitivity bound.

Three substantive findings:

1. **The bad-control comparison.** Adjusting for post-treatment variables
   that look predictive of outcome is a common modeling error. Here,
   conditioning on the recurrence mediator converts a 31% hazard
   reduction into a 10% hazard increase. The
